...
function getEquipmentSynthesis() {
  function scrapExclusiveTable(table: Element) {
    return [...table.querySelectorAll('tr')].map(tr => {
      const [,name, effect, tier1] = [...tr.querySelectorAll('td')].map(td => td && td.innerHTML.trim())
      return {
        name,
        effect,
        tier1,
  ...
  function scrapActivesPerkTable(table: Element) {
    return [...table.querySelectorAll('tr')].map(tr => {
      const [name, gear, effect] = [...tr.querySelectorAll('td')].map(td => td && td.innerHTML.trim())
      return {
        name,
        effect,
      }
  ...
  const activePerks = scrapActivesPerkTable(activeTable)
